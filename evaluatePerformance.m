function [aucs,dprimes] = evaluatePerformance(x,y,cv,method,options,nFeatures)
    ...
    for iClass = 1:nClasses
        for iTrain = 1:nTrainingExamples
            parfor iRun = 1:nRuns
                ...
                classificationValues = classifyResponses(X(chooseFeatures(X(:,cv{iClass,iTrain,iRun}),y(iClass,cv{iClass,iTrain,iRun}),[],nFeatures),:),y(iClass,:),cv{iClass,iTrain,iRun},method,options);
