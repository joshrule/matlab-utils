...
%
% imgIn: a cell array of matrices representing images
% annotation: the file holding the annotation
% CROP: a logical, if true, crop, if false, set box to 0 
%
% imgOut: an image array with some portion set to 0
    ...
    elseif CROP && exist(annotation,'file') && strcmp(ac,'.xml') % only box
        ...
        imgOut = squeeze(imgIn(str2num(b.ymin)*resize+1:str2num(b.ymax)*resize+1,str2num(b.xmin)*resize+1:str2num(b.xmax)*resize+1,:));
    ...
    elseif ~CROP && exist(annotation,'file') && strcmp(ac,'.xml') % exclude box
        ...
        for iObj = 1:length(rec.annotation.object) % exclude all objects!
            ...
            img2(str2num(b.ymin)*resize+1:str2num(b.ymax)*resize+1,str2num(b.xmin)*resize+1:str2num(b.xmax)*resize+1,:) = 0;
        end
        imgOut = img2;
    else
        imgOut = imgIn;
